...
class MyLoginRequiredMixin(AccessMixin):
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            r = requests.get('{}{}/'.format(settings.SSO_CHECK_URL, authtoken), headers={'Connection':'close'}, timeout=5).json()
            if r['code'] != 0:
                # 修改next
...
def logout(request, authtoken):
    try:
        r = requests.get('{}{}/'.format(settings.SSO_LOGOUT_URL, authtoken), headers={'Connection':'close'}, timeout=5).json()
        if r['code'] == 0:
